...
var textActions = canvas.textActions = function() {
	...
	function setCursor(index) {
		...
		// If textinput is not empty then set the index at the end of textinput
		if(!arguments.length) {
			if(empty) {
				index = 0;
			} else {
		...
		if (!cursor) {
			// If cursor does not exist, then create one using svg
		...
		if(!blinker) {
			blinker = setInterval(function() {
				// Toggle the value of DISPLAY between 'inline' and 'none' after each 600ms
	...
	function setSelection(start, end, skipInput) {
		...
		// selblock will become falls if text_selectblock does not exist
